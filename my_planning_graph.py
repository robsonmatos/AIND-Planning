...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # a_levels[0] set if all prerequisite literals for the action hold in S0. This can be accomplished by testing
        # to see if a proposed PgNode_a has prenodes that are a subset of the previous S level. Once an action node is
        # added, it MUST be connected to the S node instances in the appropriate s_level set.
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return any([True for e_add_a1 in node_a1.action.effect_add if e_add_a1 in node_a2.action.effect_rem] +
                   [True for e_add_a2 in node_a2.action.effect_add if e_add_a2 in node_a1.action.effect_rem])
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return any([True for e_add_a1 in node_a1.action.effect_add if e_add_a1 in node_a2.action.precond_neg] +
                   [True for e_rem_a1 in node_a1.action.effect_rem if e_rem_a1 in node_a2.action.precond_pos] +
                   [True for e_add_a2 in node_a2.action.effect_add if e_add_a2 in node_a1.action.precond_neg] +
                   [True for e_rem_a2 in node_a2.action.effect_rem if e_rem_a2 in node_a1.action.precond_pos])
